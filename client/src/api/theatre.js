...
//       `/api/theatres/update-theatre/${payload._id}`,  // 👈 include theatre ID
//       payload
//     );
//     return response.data;
//   } catch (err) {
...
// ✅ Update Theatre (Admin approve / Partner edit)
export const updateTheatre = async (payload) => {
  try {
    const response = await axiosInstance.put(
      "/api/theatres/update-theatre",  // no :id in URL
      payload                          // must include theatreId in body
    );
    return response.data;
  } catch (err) {
    return err.response;
